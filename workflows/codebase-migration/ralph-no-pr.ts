/**
 * Local workflow: ralph-no-pr
 *
 * Derived from Atomic's built-in Ralph workflow to support codebase-migration's
 * intermediate literal translation pass. It intentionally preserves Ralph's
 * planning, orchestration, simplification, discovery, and review loop while
 * omitting the final pull-request stage so the first migration pass cannot
 * create an intermediate PR. The final codebase-migration pass still uses the
 * imported built-in Ralph workflow with its normal PR handoff behavior.
 */

import { mkdir, mkdtemp, writeFile } from "node:fs/promises";
import { tmpdir } from "node:os";
import { dirname, join, resolve } from "node:path";
import { defineWorkflow, Type, type WorkflowRunContext, type WorkflowTaskResult } from "@bastani/workflows";
...
export default defineWorkflow("ralph-no-pr")
  .description(
    "Local Ralph-derived plan → orchestrate → simplify → parallel review loop without the final pull-request stage.",
  )
  ...
  .output("approved", Type.Optional(Type.Boolean({ description: "Whether the reviewer loop approved before this no-PR workflow completed." })))
